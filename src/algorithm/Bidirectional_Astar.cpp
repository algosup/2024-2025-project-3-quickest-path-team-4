...
optional<vector<int>> bidirectional_astar(const graph_data& graph, int start, int end, vector<int>& distances) {
    if (start == end) {
        distances[start] = 0;
    ...
    while (!forward_queue.empty() && !backward_queue.empty()) {
        ...
        for (const auto& [next, weight] : graph.adjacency[b_curr]) {
    ...
    // Update distances
    distances = std::move(g_forward);  // Use std::move explicitly
