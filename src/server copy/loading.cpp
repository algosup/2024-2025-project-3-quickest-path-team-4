...
graph_data load_graph_data(const string &file_path)
{
	...
	while (getline(file_stream, line_data))
	{
		...
		{
			gdata.node_to_index[node_a] = index_count++;
		}
		if (gdata.node_to_index.find(node_b) == gdata.node_to_index.end())
		{
			gdata.node_to_index[node_b] = index_count++;
		}

		// Update the adjacency list
		gdata.adjacency[node_a].emplace_back(node_b, time_cost);
		gdata.adjacency[node_b].emplace_back(node_a, time_cost);
	}
	file_stream.close();
	return gdata;
}