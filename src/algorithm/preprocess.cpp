...
void compress_path(graph_data* graph, int start_node) {
    auto it = graph->adjacency.find(start_node);
    if (it == graph->adjacency.end() || it->second.size() != 1) {
        return;
    }
    ...
    while (true) {
        auto curr_it = graph->adjacency.find(current_node);
        if (curr_it == graph->adjacency.end() || curr_it->second.size() != 1) {
            break;
        }

        int next_node = curr_it->second[0].first;
        int edge_weight = curr_it->second[0].second;
    ...
    if (path.size() > 2) {
        ...
        if (current_node != start_node) {  // Avoid self-loops
            ...
            if (it != graph->adjacency.end()) {
                it->second.clear();
                it->second.push_back({current_node, total_distance});
            }

            // Remove intermediate nodes from main graph
            for (size_t i = 1; i < path.size() - 1; ++i) {
                graph->adjacency.erase(path[i]);
            }
        }
    }
}
...
void preprocess(graph_data* graph, unordered_map<int, int>* single_neighbors) {
    ...
    for (const auto& [node, neighbors] : graph->adjacency) {
        if (neighbors.size() == 1) {
            compression_candidates.push_back(node);
        }
    }
